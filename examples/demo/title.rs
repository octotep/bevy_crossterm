...
pub fn setup(
    ...
    let title_pos = Position::with_xy(window.x_center() as i32 - title_sprite.x_center() as i32, window.height() as i32 / 10);

    let welcome_sprite = Sprite::new("Welcome to the bevy_crossterm demo!");
    let welcome_pos = Position::with_xy(window.x_center() as i32 - welcome_sprite.x_center() as i32, window.height() as i32 / 10 * 5);
    ...
    let explain_pos = Position::with_xy(window.x_center() as i32 - explain_sprite.x_center() as i32, window.height() as i32 / 10 * 7);

    let press_sprite = Sprite::new("Press any key to advance to the next scene");
    let press_pos = Position::with_xy(window.x_center() as i32 - press_sprite.x_center() as i32, window.height() as i32 / 10 * 9);
    ...
    commands
        .spawn_bundle(SpriteBundle {
            ...
            stylemap: stylemaps.add(StyleMap::with_attribs(Attributes::from(&[Attribute::Bold, Attribute::Underlined][..]))),
            ..Default::default()
        })
        .insert(Parent(*scene_root));
    commands
